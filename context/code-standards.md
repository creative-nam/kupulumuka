...
- **Exception: a page needing offline fallback must be a Client Component, in full, not just the interactive piece.** `shelter-results.tsx` (unit 1.3b) is the precedent — it started as a Server Component (unit 1.2) but had to move client-side because a Server Component streams pre-rendered HTML from the server; if the browser is offline during navigation, the server-side render never runs at all, so there's no way to branch to a client-side cached fallback from within a Server Component. Pages with a genuine offline-fallback requirement fetch from a corresponding API route (e.g. `/api/shelters`) on the client, rather than querying Prisma directly the way a pure Server Component would. Do not "fix" a page like this back to a Server Component to match the general rule above — check `progress-tracker.md`'s Architecture Decisions first if a page's Server/Client split looks inconsistent with this rule; it may be this exception, not a mistake.
