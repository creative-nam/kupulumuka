...
- **Exception: a page needing offline fallback keeps a thin Server Component page wrapper, and the fallback-owning piece is the Client Component child.** `app/(public)/abrigos/page.tsx` (unit 1.3b) is the precedent — it stays a thin Server Component only to validate `searchParams` (rendering the empty state when `quarteiraoId` is missing) before delegating to `ShelterResults` (`components/shelter-results.tsx`, `"use client"`), which owns the live fetch, the Dexie fallback, and the error/retry states. The page started as a pure Server Component in unit 1.2 but the rendering decision had to move to the client because a Server Component streams pre-rendered HTML from the server; if the browser is offline during navigation, the server-side render never runs at all, so there's no way to branch to a client-side cached fallback from within a Server Component. The client fallback path fetches from a corresponding API route (e.g. `/api/shelters`) rather than querying Prisma directly. Do not "fix" a page like this back to a full Server Component to match the general rule above — check `progress-tracker.md`'s Architecture Decisions first if a page's Server/Client split looks inconsistent with this rule; it may be this exception, not a mistake.
...
- **For any interactive element (button, retry action, toggle, form submit), test what it *does*, not just that it renders.** `expect(screen.getByRole("button", {name: /tentar novamente/i})).toBeInTheDocument()` proves the button exists; it does not prove clicking it does anything. This project has hit this exact gap more than once (the retry button in `shelter-results.tsx` being the clearest example) — a test asserting only presence would pass even if the click handler were entirely broken. Whenever a test covers an interactive element, include the interaction itself (e.g. `fireEvent.click(...)`) and assert the resulting state change, not just the element's initial existence.
